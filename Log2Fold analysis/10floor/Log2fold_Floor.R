...
abund_table<-read.csv("fNic.csv",row.names=1,check.names=FALSE)
meta_table<-read.csv("floor_meta.csv",row.names=1,check.names=FALSE)
...
library("ggplot2")
pdf("FloorNicReducedM.pdf")
theme_set(theme_bw())
sigtabgen = subset(sigtab, !is.na(Genus))
x = tapply(sigtabgen$log2FoldChange, sigtabgen$Genus, function(x) max(x))
x = sort(x, FALSE)
sigtabgen$Genus = factor(as.character(sigtabgen$Genus), levels=names(x))
#test2=subset(sigtabgen, sigtabgen$log2FoldChange < -6 | sigtabgen$log2FoldChange > 6)
...
ggplot(sigtabgen3, aes(y=Genus, x=log2FoldChange))+
  ggtitle("Floor Microbiome: Smoking/NonSmoking") +
...
#at.log2FC.twofold = dat[abs(dat$log2fold) >= 2.0,]
...
# 
# title = "Floor: top OTUs; Family taxa; 01-019=smoker; 101-110=non-smoker"
