...
#abund_table<-read.csv("noseReducedNict.csv",row.names=1,check.names=FALSE)
abund_table<-read.csv("nose_otu_NicV2.csv",row.names=1,check.names=FALSE)

meta_table<-read.csv("NoseNict5_2_20v2.csv",row.names=1,check.names=FALSE)
...
# mapping = aes(x = reorder(Name, Number), Number)
 ...
 ggplot(sigtabgen3, aes(x = log2FoldChange, y = Genus)) +
  ggtitle("Nose Microbiome: Smoking/NonSmoking") +
...
write.table(pGcounts,"GCounts52.csv",col.names=F , row.names = FALSE)
...
# 
# title = "Nose: top OTUs; Family taxa; 01-019=smoker; 101-110=non-smoker"
