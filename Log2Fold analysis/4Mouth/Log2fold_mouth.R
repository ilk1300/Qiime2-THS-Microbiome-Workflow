...
abund_table<-read.csv("mouthOTU.csv",row.names=1,check.names=FALSE)
meta_table<-read.csv("meta.csv",row.names=1,check.names=FALSE)
...
ggplot(sigtabgen3, aes(y=Genus, x=log2FoldChange))+
  ggtitle("Mouth Microbiome: Smoking/Non-smoking with Nicotine Con. (M)") +
...
# 
# title = "Mouth: top OTUs; Family taxa; 01-019=smoker; 101-110=non-smoker"
...
# 
# test=as.data.frame()abund_table