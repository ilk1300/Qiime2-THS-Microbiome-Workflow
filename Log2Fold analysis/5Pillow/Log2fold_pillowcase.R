...
abund_table<-read.csv("pOTU.csv",row.names=1,check.names=FALSE)
meta_table<-read.csv("pillow_meta.csv",row.names=1,check.names=FALSE)
...
ggplot(sigtabgen3, aes(y=Genus, x=log2FoldChange))+
  ggtitle("Pillowcase Microbiome: Smoking/Non-smoking") +
...
# 
# title = "Pillowcase: top OTUs; Family taxa; 01-019=smoker; 101-110=non-smoker"
